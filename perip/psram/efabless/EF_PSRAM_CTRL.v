...
module PSRAM_READER (
  ...
  always @*
    case (state)
      IDLE: if (rd) nstate = READ;
 else nstate = IDLE;
      READ: if (done) nstate = IDLE;
 else nstate = READ;
    endcase
  ...
  always @(posedge clk)
    if (counter >= 20 && counter <= FINAL_COUNT)
      if (sck) data[byte_index] <= {data[byte_index][3:0], din};  // Optimize!
  ...
  generate
    genvar i;
    for (i = 0; i < 4; i = i + 1) assign line[i*8+7:i*8] = data[i];
  endgenerate
...
module PSRAM_WRITER (
  ...
  always @(posedge clk or negedge rst_n)
    if (!rst_n) saddr <= 24'b0;
    else if ((state == IDLE) && wr) saddr <= addr;
