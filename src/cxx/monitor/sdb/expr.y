...
unary:
  ...
  | '*' unary %prec DEREF { extern bool in_mrom(paddr_t); extern bool in_sram(paddr_t); if (in_mrom($2) || in_sram($2)) { $$ = vaddr_read($2, sizeof(word_t)); } else { $$ = 0xdeadbeef; runtime_error = true; sdb_exprerror("invalid memory access"); } } /* 解引用 */
  ;
