...
impl Memory {
    ...
    pub fn load_u8(&self, addr: u32) -> miette::Result<u8> {
        match addr {
            ...
            a if a.wrapping_sub(PSRAM_BASE) < PSRAM_SIZE => {
                Ok(self.psram.lock().unwrap()[(a - PSRAM_BASE) as usize])
            }
    ...
    pub fn store_u8(&self, addr: u32, val: u8) -> miette::Result<()> {
        match addr {
            a if a.wrapping_sub(PSRAM_BASE) < PSRAM_SIZE => {
                self.psram.lock().unwrap()[(a - PSRAM_BASE) as usize] = val;
                Ok(())
            }
            a if a.wrapping_sub(SRAM_BASE) < SRAM_SIZE => {
