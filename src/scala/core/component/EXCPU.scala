...
class EXCPUOutput extends NPCBundle {
  val mcause = UInt(dataBits.W)
  val mtval = UInt(dataBits.W)
}

class EXCPUInput extends NPCBundle {
  val ifu = IFUExceptionType()
  val ifuEn = Bool()
  val ifuXtval = UInt(dataBits.W)
  val cu = CUExceptionType()
  val cuEn = Bool()
  val cuXtval = UInt(dataBits.W)
  val lsu = MemUExceptionType()
  val lsuEn = Bool()
  val lsuXtval = UInt(dataBits.W)
  val pc = UInt(dataBits.W)
}

class EXCPU extends Module {
  val io = IO(new Bundle {
    val in = Flipped(DecoupledIO(new EXCPUInput))
    val out = DecoupledIO(new EXCPUOutput)
  })
  ...
  // IFU > CU > LSU
  private val mcause = MuxCase(
    0.U,
    Seq(
      ins.ifuEn -> MuxLookup(ins.ifu, 0.U)(ifuExceptionMcauseMap.map {
        case (k, v) => k -> v
      }),
      ins.cuEn -> MuxLookup(ins.cu, 0.U)(cuExceptionMcauseMap.map {
        case (k, v) => k -> v
      }),
      ins.lsuEn -> MuxLookup(ins.lsu, 0.U)(lsuExceptionMcauseMap.map {
        case (k, v) => k -> v
      })
    )
  )
